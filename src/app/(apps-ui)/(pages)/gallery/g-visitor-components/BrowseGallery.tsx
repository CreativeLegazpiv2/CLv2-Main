    ...
    return (
        <div className="inline-block px-3 max-sm:px-0 w-full max-w-lg"> {/* Remove padding on mobile */}
            <Link href={`/gallery/artwork/${slug}`} passHref>
                <motion.article
                    className="rounded-2xl w-full flex flex-col cursor-pointer"
                    initial={{ opacity: 0, y: 50 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5 }}
                >
                    <div className="relative overflow-hidden min-h-[40dvh] w-full rounded-lg">
                        <motion.img
                            src={image_path}
                            alt={`${title} artwork`}
...
const CustomArrow = ({ onClick, direction }: { onClick?: () => void; direction: 'prev' | 'next' }) => (
    <button
        onClick={onClick}
        className={`absolute z-20 top-1/2 -translate-y-1/2 ${direction === 'prev' ? 'left-4' : 'right-4'
